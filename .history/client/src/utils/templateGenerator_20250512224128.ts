...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  // Set switcher (removed, not needed when all sets are visible)
  const setSwitcher = '';
  ...
  return `
    <div class="gallery product-gallery">
      ...
      <style>
        ...
        .thumbnail-set { display: flex; justify-content: center; gap: 0.5rem; flex-wrap: wrap; margin-bottom: 0.5rem; background: #fff; border-radius: var(--radius, 4px); border: 1px solid var(--frame-border, #e5e7eb); padding: 0.5rem 0; }
        .thumbnail { width: 80px; height: 80px; border: 2px solid var(--frame-border, #e5e7eb); border-radius: var(--radius, 4px); overflow: hidden; cursor: pointer; transition: all 0.2s; flex-shrink: 0; background: #f9f9f9; }
