...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  // Generate navigation arrows with proper labels and icons
  ...
  // Generate navigation logic for all images
  const navigationLogic = images.map((_, idx) => {
    const prevIdx = idx === 0 ? images.length : idx;
    const nextIdx = idx === images.length - 1 ? 1 : idx + 2;
    return `
    #img${idx + 1}:checked ~ .gallery-container #main${idx + 1} { opacity: 1; }
    #img${idx + 1}:checked ~ .thumbnails label[for="img${idx + 1}"] { border-color: var(--accent-color, #4CAF50); }
    #img${idx + 1}:checked ~ .gallery-container .prev { --prev: img${prevIdx}; }
    #img${idx + 1}:checked ~ .gallery-container .next { --next: img${nextIdx}; }
    `;
  }).join('\n');
