...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  const arrows = images.length > 1 ? images.map((_, idx) => `
  ...
  // CSS selectors for showing main images
  ...
  <div class="gallery product-gallery">
    ...
    <style>
      ...
      ${activeThumbSelectors} { border-color: var(--primary, #3b82f6); }
      ...
      ${Array.from({length: setCount}, (_, i) => 
        `#set${i + 1}:checked ~ .set-switcher label[for="set${i + 1}"] { background: var(--primary, #3b82f6); opacity: 1; }`
      ).join('\n')}
    </style>
  </div>`;
}