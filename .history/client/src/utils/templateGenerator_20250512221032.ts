...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  // Navigation arrows for each image (circular)
  const navigationArrows = images.map((_, idx) => {
    const prevIdx = idx === 0 ? images.length : idx;
    const nextIdx = idx === images.length - 1 ? 1 : idx + 2;
    // Calculate which set the prev/next image is in
    const prevSet = Math.floor((prevIdx - 1) / setSize) + 1;
    const nextSet = Math.floor((nextIdx - 1) / setSize) + 1;
    let arrows = '';
    // When navigating, also check the correct set radio
    arrows += `<label class="gallery-arrow prev" for="img${prevIdx}" onclick="document.getElementById('set${prevSet}').checked = true" title="Previous"></label>`;
    arrows += `<label class="gallery-arrow next" for="img${nextIdx}" onclick="document.getElementById('set${nextSet}').checked = true" title="Next"></label>`;
    return `<div class="gallery-arrows" style="display:none" data-for="img${idx + 1}">${arrows}</div>`;
  }).join('\n');
  ...
  // Set switcher
  const setSwitcher = sets.length > 1 ? `
    <div class="set-switcher">
      ${sets.map((_, i) => `<label for="set${i + 1}"></label>`).join('')}
    </div>
  ` : '';
  ...
  // Show correct thumbnail set for each set radio
  const showSetSelectors = sets.map((_, i) =>
    `#set${i + 1}:checked ~ .thumbnail-sets .set${i + 1}`
  ).join(',\n');
  // Show correct set switcher dot
  const showDotSelectors = sets.map((_, i) =>
    `#set${i + 1}:checked ~ .set-switcher label[for="set${i + 1}"]`
  ).join(',\n');
  // Show only the arrows for the active image
  const showArrowSelectors = images.map((_, idx) =>
    `#img${idx + 1}:checked ~ .gallery-container .gallery-arrows[data-for="img${idx + 1}"]`
  ).join(',\n');
  ...
  return `
    <div class="gallery product-gallery">
      ...
      <style>
        ...
        .gallery-arrows { position: absolute; top: 0; left: 0; right: 0; bottom: 0; pointer-events: none; }
        .gallery-arrow { position: absolute; top: 50%; transform: translateY(-50%); width: 40px; height: 40px; background: rgba(255,255,255,0.85); border: 1px solid var(--frame-border, #e5e7eb); border-radius: 50%; cursor: pointer; z-index: 10; pointer-events: auto; box-shadow: 0 2px 8px rgba(0,0,0,0.10); opacity: 0.7; display: flex; align-items: center; justify-content: center; transition: opacity 0.2s, background 0.2s; }
        .gallery-arrow.prev { left: 10px; }
        .gallery-arrow.next { right: 10px; }
        .gallery-arrow:hover { opacity: 1; background: #fff; }
        .gallery-arrow:before { display: none; }
        .gallery-arrow svg, .gallery-arrow span { font-size: 22px; color: #222; }
        ...
        .thumbnail { width: 80px; height: 80px; border: 2px solid var(--frame-border, #e5e7eb); border-radius: var(--radius, 4px); overflow: hidden; cursor: pointer; transition: all 0.2s; flex-shrink: 0; }
        .thumbnail img { width: 100%; height: 100%; object-fit: cover; }
        .thumbnail:hover { border-color: var(--primary, #3b82f6); }
        ${showThumbSelectors} { border-color: var(--primary, #3b82f6); }
        ${showSetSelectors} { display: flex; }
        .set-switcher { display: flex; justify-content: center; gap: 0.5rem; margin-top: 0.5rem; }
        .set-switcher label { width: 8px; height: 8px; border-radius: 50%; background: var(--frame-border, #e5e7eb); cursor: pointer; transition: background-color 0.3s; opacity: 0.7; }
        .set-switcher label:hover { opacity: 1; }
        ${showDotSelectors} { background: var(--primary, #3b82f6); opacity: 1; }
        /* Only show arrows for the active image */
        .gallery-arrows { display: none; }
        ${showArrowSelectors} { display: block; }
      </style>
    </div>
  `;
}