...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  // Navigation arrows: only one set, dynamically update targets with CSS
  const navigationArrows = `
    <label class="gallery-arrow prev" for="img1" title="Previous">&#8592;</label>
  ...
  // Dynamic prev/next for arrows using CSS only
  const arrowPrevSelectors = images.map((_, idx) => {
    const prevIdx = idx === 0 ? images.length : idx;
    const prevSet = Math.floor((prevIdx - 1) / setSize) + 1;
    return `#img${idx + 1}:checked ~ .gallery-container .gallery-arrow.prev { for: img${prevIdx}; }\n#img${idx + 1}:checked ~ .gallery-container .gallery-arrow.prev { onclick: document.getElementById('set${prevSet}').checked = true; }`;
  }).join('\n');
  const arrowNextSelectors = images.map((_, idx) => {
    const nextIdx = idx === images.length - 1 ? 1 : idx + 2;
    const nextSet = Math.floor((nextIdx - 1) / setSize) + 1;
    return `#img${idx + 1}:checked ~ .gallery-container .gallery-arrow.next { for: img${nextIdx}; }\n#img${idx + 1}:checked ~ .gallery-container .gallery-arrow.next { onclick: document.getElementById('set${nextSet}').checked = true; }`;
  }).join('\n');
