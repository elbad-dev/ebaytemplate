  ...
  if (data.rawHtml) {
    ...
    if (data.images && data.images.length > 0) {
      ...
      if (gallerySection) {
        ...
        gallerySection.replaceWith(newGallery.querySelector('.gallery'));
      }
...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  // Navigation arrows for main image - ensuring prev/next arrows for each image
  const arrows = images.map((_, idx) => {
    const hasPrev = idx > 0;
    const hasNext = idx < images.length - 1;
    return `
      ${hasPrev ? `<label class="gallery-arrow prev" for="img${idx}"></label>` : ''}
      ${hasNext ? `<label class="gallery-arrow next" for="img${idx + 2}"></label>` : ''}
    `;
  }).join('\n');

  // Main images with loading="lazy" for better performance
  ...
  // CSS selectors for showing active images
  ...
  const showPrevSelectors = images.slice(1).map((_, idx) =>
    `#img${idx + 1}:checked ~ .gallery-container .prev[for="img${idx}"]`
  ).join(',\n');

  // Combine all elements into the gallery structure
  return `
    <div class="gallery product-gallery">
      ...
      <style>
        ${showMainSelectors} { display: block; }
        ${showThumbSelectors} { border-color: var(--accent-color); }
        ${showNextSelectors} { display: flex; }
        ${showPrevSelectors} { display: flex; }
      </style>
    </div>
  `;
}