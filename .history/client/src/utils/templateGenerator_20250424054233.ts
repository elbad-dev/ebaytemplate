...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  // 1. Radio inputs
  const radios = images.map((_, idx) =>
    `<input type="radio" name="gallery" id="img${idx + 1}"${idx === 0 ? ' checked' : ''}>`
  ).join('\n');

  // 2. Navigation arrows
  const prevArrows = images.map((_, idx) =>
    `<label class="gallery-arrow prev" for="img${idx + 1}"></label>`
  ).join('\n');
  const nextArrows = images.map((_, idx) =>
  ...
  const thumbnails = images.map((img, idx) =>
    `<label class="thumbnail" for="img${idx + 1}"><img src="${img.url}" alt="Thumbnail ${idx + 1}"></label>`
  ).join('\n');

  // 5. CSS selectors for showing main images
  const showMainSelectors = images.map((_, idx) =>
    `#img${idx + 1}:checked ~ .gallery-container #main${idx + 1}`
  ).join(',\n');

  // 6. CSS selectors for next/prev arrows
  const showNextSelectors = images.slice(0, -1).map((_, idx) =>
    `#img${idx + 1}:checked ~ .gallery-container .next[for="img${idx + 2}"]`
  ).join(',\n');
  const showPrevSelectors = images.slice(1).map((_, idx) =>
    `#img${idx + 2}:checked ~ .gallery-container .prev[for="img${idx + 1}"]`
  ).join(',\n');
  ...
  <style>
    ...
    .gallery-container { position: relative; width: 100%; min-height: 400px; }
    .gallery-item { position: absolute; top: 0; left: 0; width: 100%; height: auto; opacity: 0; transition: opacity 0.3s; z-index: 1; }
    ${showMainSelectors} { opacity: 1; z-index: 2; }
    .gallery-arrow { display: none; position: absolute; top: 50%; width: 40px; height: 40px; background: #fff; border-radius: 50%; border: 1px solid #ccc; cursor: pointer; align-items: center; justify-content: center; }
    .gallery-arrow.prev { left: 10px; }
    .gallery-arrow.next { right: 10px; }
    .gallery-arrow:before { content: ''; display: block; width: 10px; height: 10px; border-style: solid; border-width: 3px 3px 0 0; border-color: #333; }
    .gallery-arrow.prev:before { transform: rotate(-135deg); margin-left: 5px; }
    .gallery-arrow.next:before { transform: rotate(45deg); margin-right: 5px; }
    ${showNextSelectors} { display: block; }
    ${showPrevSelectors} { display: block; }
    .thumbnail-sets { margin-top: 20px; }
    .thumbnail-set { display: flex; gap: 10px; }
    .thumbnail { width: 60px; height: 60px; border: 1px solid #ddd; border-radius: 8px; overflow: hidden; cursor: pointer; }
