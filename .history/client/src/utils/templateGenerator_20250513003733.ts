  ...
  return `
    <div class="gallery product-gallery">
      ...
      <style>
        ...
        .thumbnail-set { display: flex; justify-content: center; gap: 0.5rem; flex-wrap: wrap; margin-bottom: 0.5rem; }        .thumbnail { 
          width: 80px; 
          height: 80px; 
        ...
        ${images.map((_, idx) => 
          ...
          #img${idx + 1}:checked ~ .thumbnail-sets .thumbnail[for="img${idx + 1}"] img {
            transform: scale(1.05);
          }`
        ).join('\n')}
      </style>
