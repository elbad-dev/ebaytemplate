...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  const mainImages = images.map((img, idx) => `
    <img src="${img.url}" alt="${img.alt || `Detail ${idx + 1}`}" class="gallery-item" loading="${idx === 0 ? 'eager' : 'lazy'}">
  `).join('\n');
  ...
  // Navigation states for each image
  const navigationStates = images.map((_, idx) => {
    const prevIdx = idx === 0 ? images.length : idx;
    const nextIdx = idx === images.length - 1 ? 1 : idx + 2;
    return `
      #img${idx + 1}:checked ~ .gallery-container .gallery-item { display: none; }
      #img${idx + 1}:checked ~ .gallery-container .gallery-item:nth-child(${idx + 1}) { display: block; }
      #img${idx + 1}:checked ~ .gallery-container .prev { --prev-image: "img${prevIdx}"; }
      #img${idx + 1}:checked ~ .gallery-container .next { --next-image: "img${nextIdx}"; }
      #img${idx + 1}:checked ~ .thumbnails .thumbnail { border-color: transparent; }
      #img${idx + 1}:checked ~ .thumbnails label[for="img${idx + 1}"] { border-color: var(--accent-color, #4CAF50); }
    `;
  }).join('\n');
      ...
      <div class="gallery-container">
        ...
        ${images.length > 1 ? `
          <div class="gallery-navigation">
            <label class="gallery-arrow prev" for="img${images.length}">
              <span class="arrow-icon">←</span>
            </label>
            <label class="gallery-arrow next" for="img2">
              <span class="arrow-icon">→</span>
            </label>
          </div>
        ` : ''}
