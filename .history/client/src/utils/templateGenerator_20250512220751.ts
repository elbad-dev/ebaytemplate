...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  const navigationArrows = images.map((_, idx) => {
    ...
    let arrows = '';
    if (idx > 0) arrows += `<label class="gallery-arrow prev" for="img${prevIdx}" title="Previous"></label>`;
    if (idx < images.length - 1) arrows += `<label class="gallery-arrow next" for="img${nextIdx}" title="Next"></label>`;
    // For first and last, ensure circular
    if (idx === 0) arrows += `<label class="gallery-arrow prev" for="img${images.length}" title="Previous"></label>`;
    if (idx === images.length - 1) arrows += `<label class="gallery-arrow next" for="img1" title="Next"></label>`;
    return arrows;
  }).join('\n');
