...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  // Navigation arrows: statically generate as in the example, no JS
  let navigationArrows = '';
  for (let i = 0; i < images.length; i++) {
    // Next arrow (skip for last image)
    if (i < images.length - 1) {
      navigationArrows += `\n<label class="gallery-arrow next" for="img${i + 2}"></label>`;
    }
    // Prev arrow (skip for first image)
    if (i > 0) {
  ...
  return `
    <div class="gallery product-gallery">
      ...
      <style>
        ...
        /* Show only the correct arrows for each image */
        ${images.map((_, idx) => {
          let selectors = [];
          if (idx < images.length - 1) selectors.push(`#img${idx + 1}:checked ~ .gallery-container .gallery-arrow.next[for="img${idx + 2}"]`);
          if (idx > 0) selectors.push(`#img${idx + 1}:checked ~ .gallery-container .gallery-arrow.prev[for="img${idx}"]`);
