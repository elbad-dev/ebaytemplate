...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  // Navigation arrows (single set, dynamically updates targets)
  const navigationArrows = `
    <div class="gallery-arrows">
      <label class="gallery-arrow prev" id="gallery-prev-arrow" title="Previous">&#8592;</label>
      <label class="gallery-arrow next" id="gallery-next-arrow" title="Next">&#8594;</label>
    </div>
  `;
        ...
        (function() {
          ...
          function getCheckedIndex() {
            for (let i = 1; i <= total; i++) {
              if (document.getElementById('img'+i).checked) return i;
            }
            return 1;
          }
          function getSetIndex(imgIdx) {
            return Math.floor((imgIdx-1)/setSize)+1;
          }
          function updateArrows() {
            const idx = getCheckedIndex();
            let prevIdx = idx === 1 ? total : idx - 1;
            let nextIdx = idx === total ? 1 : idx + 1;
            prevArrow.setAttribute('for', 'img'+prevIdx);
            nextArrow.setAttribute('for', 'img'+nextIdx);
            prevArrow.onclick = function() {
              document.getElementById('img'+prevIdx).checked = true;
              document.getElementById('set'+getSetIndex(prevIdx)).checked = true;
            };
            nextArrow.onclick = function() {
              document.getElementById('img'+nextIdx).checked = true;
              document.getElementById('set'+getSetIndex(nextIdx)).checked = true;
            };
          }
          document.querySelectorAll('input[name="gallery"]').forEach(radio => {
