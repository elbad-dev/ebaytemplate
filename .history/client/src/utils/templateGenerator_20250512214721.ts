...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  // Generate navigation arrows for circular navigation
  const navigationArrows = images.length > 1 ? `
  ...
  // Generate main images and thumbnails
  ...
  // Generate navigation logic for circular motion
  const circularNavigation = images.map((_, idx) => {
    const prevIdx = idx === 0 ? images.length : idx;
    const nextIdx = idx === images.length - 1 ? 1 : idx + 2;
    return `
    #img${idx + 1}:checked ~ .gallery-container .prev { --prev-target: img${prevIdx}; }
    #img${idx + 1}:checked ~ .gallery-container .next { --next-target: img${nextIdx}; }
    #img${idx + 1}:checked ~ .gallery-container #main${idx + 1} { opacity: 1; pointer-events: auto; }
    `;
  }).join('\n');
  ...
  return `
    <div class="gallery product-gallery">
      ...
      <style>
        ...
        input[type="radio"]:checked + .gallery-container .gallery-item {
          opacity: 1;
          pointer-events: auto;
        }
        
        input[type="radio"]:checked ~ .thumbnails label[for="img1"] {
