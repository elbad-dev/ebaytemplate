...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  const thumbnailSets = sets.map((set, setIdx) =>
    `<div class="thumbnail-set set${setIdx + 1}" style="background: #fff; border: 1px solid #e5e7eb; border-radius: 4px; padding: 8px 0; box-shadow: 0 1px 2px rgba(0,0,0,0.03);">
      ${set.map((img, imgIdx) => {
