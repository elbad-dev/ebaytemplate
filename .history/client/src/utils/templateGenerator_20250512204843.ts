...
export function generateGallerySection(images: {url: string, alt?: string}[]): string {
  ...
  // Generate navigation arrows for each image
  const navigationArrows = images.map((_, idx) => {
    const prevIdx = idx;
    const nextIdx = idx + 2;
    return `
      ${idx > 0 ? `<label class="gallery-arrow prev" for="img${prevIdx}"></label>` : ''}
      ${idx < images.length - 1 ? `<label class="gallery-arrow next" for="img${nextIdx}"></label>` : ''}
    `;
  }).join('\n');
  ...
  return `
    <div class="gallery product-gallery">
      ...
      <style>
        ...
        /* Show states */
        .gallery-item { display: none; opacity: 0; transition: opacity 0.3s ease; }
        .gallery-arrow { display: none; }
        .thumbnail { border-color: transparent; }
        ${showMainSelectors} { display: block; opacity: 1; }
        ${showThumbSelectors} { 
          border-color: var(--accent-color, #0066c0);
          transform: translateY(-2px);
          box-shadow: 0 4px 8px rgba(0,0,0,0.1);
        }
        ${showArrowSelectors} { display: flex; }
      </style>
    </div>
  `;
}