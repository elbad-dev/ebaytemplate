...
const CompanyEditor: React.FC<CompanyEditorProps> = ({ data, onUpdate, onEditSvg }) => {
  ...
  const [logoSize, setLogoSize] = React.useState(data.logoSize || '1.5rem');
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="mb-4">
            ...
            <div className="mb-2 flex items-center">
              <div className="border border-gray-200 rounded-md p-2 bg-white max-w-[180px] mr-2">
                {logo.includes('<svg') ? (
                  ...
                  <img src={logo} alt="Logo" style={{ height: logoSize, width: 'auto', objectFit: 'contain' }} />
                )}
              </div>
              <Button
                variant="outline"
        ...
        <div className="mt-2">
          ...
          <Select value={logoSize} onValueChange={val => { setLogoSize(val); onUpdate({ logoSize: val }); }}>
            ...
            <SelectContent>
              ...
              <SelectItem value="2.5rem">2.5rem</SelectItem>
              <SelectItem value="3rem">3rem</SelectItem>
              <SelectItem value="3.5rem">3.5rem</SelectItem>
              <SelectItem value="4rem">4rem</SelectItem>
              <SelectItem value="5rem">5rem</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <p className="mt-1 text-xs text-gray-500">Upload a new logo to replace the current one. You can also adjust the logo size below.</p>
      </div>
      {/* Company Name */}
      <div className="mb-4">
