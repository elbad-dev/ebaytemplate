...
// Add a helper to preview the gallery section directly (for debugging or visual feedback)
function GalleryPreview({ images }: { images: { url: string; alt?: string }[] }) {
  return (
    <div dangerouslySetInnerHTML={{ __html: generateGallerySection(images) }} />
  );
}
...
function replaceGallerySection(html: string, images: { url: string; alt?: string }[]): string {
  const galleryRegex = /<div\s+class="gallery"[^>]*>[\s\S]*?<\/div>/i;
  if (galleryRegex.test(html)) {
    return html.replace(galleryRegex, generateGallerySection(images));
  }
  return html;
}
...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  // Always preserve original template structure and update only changed sections
  useEffect(() => {
    if (template && template.html) {
      // First handle gallery section separately
      let updatedHtml = replaceGallerySection(template.html, templateData.images);
      
      // Then update other sections while preserving structure
      const doc = new DOMParser().parseFromString(updatedHtml, 'text/html');
      
      // Update product info sections
      if (templateData.title) {
        const titleEl = doc.querySelector('.product-title, .product-name');
        if (titleEl) titleEl.textContent = templateData.title;
      }
      
      if (templateData.description) {
        const descEl = doc.querySelector('.product-description');
        if (descEl) descEl.innerHTML = templateData.description;
      }
      
      // Update company sections
      if (templateData.company_name) {
        const companyEl = doc.querySelector('.company-name');
        if (companyEl) companyEl.textContent = templateData.company_name;
      }
      
      // Update specs section if exists
      if (templateData.specs.length > 0) {
        const specsEl = doc.querySelector('.specs-container');
        if (specsEl) {
          specsEl.innerHTML = templateData.specs.map(spec => `
            <div class="spec-item">
