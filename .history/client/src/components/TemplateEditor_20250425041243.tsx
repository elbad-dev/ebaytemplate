...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  // Ensure that when editing sections like Info, Desc, Specs, and About, the container, style, and font are preserved.
  // Update the `handleSectionUpdate` function to enforce this constraint.
  const handleSectionUpdate = (updates: Partial<TemplateData>) => {
    const updatedData = { ...templateData, ...updates };

    // Ensure styles and structure are preserved
    const preserveStyles = (html: string) => {
      const parser = new DOMParser();
      const doc = parser.parseFromString(html, 'text/html');

      // Example: Ensure container classes remain intact
      const containers = doc.querySelectorAll('.info-container, .desc-container, .specs-container, .about-container');
      containers.forEach(container => {
        const originalClasses = container.className;
        container.className = originalClasses; // Preserve original classes
      });
    ...
    generatePreview({
      ...updatedData,
      html: preserveStyles(generatedHtml),
    });
