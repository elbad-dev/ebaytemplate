...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  // --- DeepL Translation Logic ---
  const [isTranslating, setIsTranslating] = useState(false);
  const [targetLang, setTargetLang] = useState<string>('DE');
  const deeplApiKey = '86cb9c06-b70d-4b21-9151-b66338f71a0d:fx';

  // Helper: Translate text using DeepL API
  async function translateText(text: string, targetLang: string = 'DE'): Promise<string> {
    if (!text) return '';
    try {
  ...
  // Translate all template fields
  const handleTranslateTemplate = async (targetLang: string = 'DE') => {
    setIsTranslating(true);
    try {
      // Translate main fields
      const [title, subtitle, description, priceNote, cartButtonText, buyButtonText, descriptionTitle] = await Promise.all([
        translateText(templateData.title, targetLang),
        translateText(templateData.subtitle, targetLang),
        translateText(templateData.description, targetLang),
        translateText(templateData.priceNote, targetLang),
        translateText(templateData.cartButtonText, targetLang),
        translateText(templateData.buyButtonText, targetLang),
        translateText(templateData.descriptionTitle, targetLang),
      ]);
      // Translate tags
      const tags = templateData.tags
        ? await Promise.all(templateData.tags.map(async tag => ({ ...tag, label: await translateText(tag.label, targetLang) })))
        : [];
      // Translate specs
      const specs = templateData.specs
        ? await Promise.all(templateData.specs.map(async spec => ({
            ...spec,
            label: await translateText(spec.label, targetLang),
            value: await translateText(spec.value, targetLang),
          })))
        : [];
      // Translate company info
      const companyInfo = templateData.companyInfo
        ? await Promise.all(templateData.companyInfo.map(async info => ({
            ...info,
            title: await translateText(info.title, targetLang),
            description: await translateText(info.description, targetLang),
          })))
        : [];
      // Update templateData and regenerate preview
      const updated = {
        ...templateData,
        title,
        subtitle,
      ...
      generatePreview(updated, true); // Ensure preview uses freshly generated HTML after translation
      toast({ title: 'Translation complete', description: 'Template translated successfully.' });
    } catch (error) {
      toast({ title: 'Translation failed', description: 'Could not translate template.', variant: 'destructive' });
    } finally {
  ...
  const generatePreview = (templateData: TemplateData, forceFresh: boolean = false) => {
    // Always use a fresh template as the base if forceFresh is true
    let baseHtml = forceFresh ? generateTemplate(templateData) : (generatedHtml || template?.html || generateTemplate(templateData));
    if (!baseHtml) {
    ...
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      <header className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-2 mt-4">
          ...
          <Button onClick={() => handleTranslateTemplate(targetLang)} className="py-4" disabled={isTranslating}>
            {isTranslating ? 'Translating...' : 'Translate Template'}
          </Button>
      ...
      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded text-blue-900 text-sm">
        <strong>Note:</strong> Only editable fields (title, description, specs, company info, etc.) will be translated. <br />
        Static text in the uploaded template that is not editable will remain unchanged. To translate more content, ensure it is parsed and editable in the template editor.
      </div>
