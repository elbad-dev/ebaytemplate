...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  const generatePreview = (templateData: TemplateData) => {
    ...
    try {
      ...
      // Update specifications (regenerate .specs-container, .specifications, .tech-specs)
      ...
      // Update product details (regenerate .product-info .card:first-child for title, subtitle, price)
      const productInfo = doc.querySelector('.product-info .card:first-child');
      if (productInfo) {
        let html = '';
        if (templateData.title) {
          html += `<h2 class="product-title">${templateData.title}</h2>`;
        }
        if (templateData.subtitle) {
          html += `<div class="company-slogan">${templateData.subtitle}</div>`;
        }
        if (templateData.price) {
          const currencySymbol = getCurrencySymbol(templateData.currency);
          html += `<div class="price">${currencySymbol}${templateData.price}</div>`;
        }
        productInfo.innerHTML = html;
      }

      // Update product description (regenerate .product-description, .description, .description-text, .item-description)
      const descContainer = doc.querySelector('.product-description, .description, .description-text, .item-description');
      if (descContainer) {
        descContainer.innerHTML = templateData.description || '';
      }
