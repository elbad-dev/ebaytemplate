...
const PreviewPanel: React.FC<PreviewPanelProps> = ({ 
  ...
  const [selectedElement, setSelectedElement] = useState<StyledTextElement | null>(null);
    ...
    if (iframeRef.current) {
      ...
      if (iframeDoc) {
        ...
        iframeDoc.write(`
          ...
          <html>
            <head>
              ${sanitizedHtml.match(/<head[^>]*>([\s\S]*?)<\/head>/i)?.[1] || ''}
            </head>
            <body>
              ${sanitizedHtml.match(/<body[^>]*>([\s\S]*?)<\/body>/i)?.[1] || sanitizedHtml}
            </body>
          </html>
        `);
        iframeDoc.close();
  ...
  const updateFullPreviewContent = () => {
    if (fullPreviewRef.current) {
      ...
      if (iframeDoc) {
        // Preserve the original HTML structure for full preview as well
        const sanitizedHtml = sanitizeHtml(html);
        iframeDoc.open();
        iframeDoc.write(`
          <!DOCTYPE html>
  ...
  useEffect(() => {
    ...
    const updateContent = () => {
      ...
      iframe.contentDocument.write(html || getEmptyPreview());
