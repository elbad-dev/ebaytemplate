...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  useEffect(() => {
    if (lastEditedSection && generatedHtml) {
      setTimeout(() => {
        const el = document.getElementById(lastEditedSection);
        if (el) {
          el.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
        setLastEditedSection(null); // Reset after scroll so it doesn't trigger on every update
      }, 100);
    }
  ...
  const handleTemplateImport = (data: TemplateData) => {
    setTemplateData(prev => {
      return {
        ...prev,
        // Only update tags if the user hasn't changed them (empty or default)
        tags: (prev.tags && prev.tags.length > 0) ? prev.tags : data.tags,
        // Only update priceNote if the user hasn't changed it
