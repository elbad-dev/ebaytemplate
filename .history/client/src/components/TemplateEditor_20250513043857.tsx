...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  const handleSectionEdit = (sectionId: string, newContent: string) => {
    ...
    switch (sectionId) {
      case 'product-title':
        sectionUpdates.title = newContent;
        break;
      ...
      case 'tech-specs':
        ...
        const specItems = tempDiv.querySelectorAll('li');
        sectionUpdates.specs = Array.from(specItems).map(item => item.textContent || '');
        break;
      case 'company-info':
        // Extract company info from HTML content
        const companyDiv = document.createElement('div');
        companyDiv.innerHTML = newContent;
        const companyItems = companyDiv.querySelectorAll('li, p');
        sectionUpdates.companyInfo = Array.from(companyItems).map(item => item.textContent || '');
        break;
    }
