...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  const handleSectionUpdate = (updates: Partial<TemplateData>) => {
    setTemplateData(prev => {
      ...
      generatePreview(updated);  // Generate preview immediately after state update
      return updated;
    });
  };
  ...
  const generatePreview = (templateData: TemplateData) => {
    ...
    try {
      ...
      updateContent(['.product-title', '.product-name', '.title', 'h1.title'], templateData.title);
      updateContent(['.company-name', '.brand-name'], templateData.company_name);
      updateContent(['.company-slogan', '.brand-slogan', '.subtitle'], templateData.subtitle);

      // Format and update price
      if (templateData.price) {
        const currencySymbol = getCurrencySymbol(templateData.currency);
        updateContent(['.price', '.product-price'], `${currencySymbol}${templateData.price}`);
      }
      ...
      if (templateData.companyInfo?.length > 0) {
        const companySections = doc.querySelectorAll('.company-section, .info-section');
      ...
      if (templateData.images?.length > 0) {
        const updatedHtml = replaceGallerySection(doc.documentElement.outerHTML, templateData.images);
        doc = parser.parseFromString(updatedHtml, 'text/html');
      }
