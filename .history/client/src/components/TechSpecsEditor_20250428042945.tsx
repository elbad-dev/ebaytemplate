...
const TechSpecsEditor: React.FC<EditorSectionProps> = ({ data, onUpdate }) => {
  ...
  // Add state for technical image
  const [techImage, setTechImage] = useState(data.techImage || '');

  // Handler for image URL change
  const handleTechImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setTechImage(e.target.value);
    onUpdate({ techImage: e.target.value });
  };
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="mb-4">
        <label className="block text-xs text-gray-600 mb-1">Technical Data Image</label>
        <Input
          type="text"
          className="w-full p-2 border border-gray-300 rounded-md text-sm"
          value={techImage}
        ...
        {techImage && (
          <div className="mt-2 flex justify-center">
            <img
              src={techImage}
              alt="Technical Data"
              style={{ maxWidth: '100%', height: 'auto', borderRadius: '16px', objectFit: 'contain' }}
            />
          </div>
        )}
      </div>
