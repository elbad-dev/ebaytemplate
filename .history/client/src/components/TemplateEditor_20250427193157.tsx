...
export default function TemplateEditor({ template, data, onSave, onBack }: TemplateEditorProps) {
  ...
  const [generatedHtml, setGeneratedHtml] = useState<string>(data.rawHtml || '');
  ...
  // Initialize preview document when HTML changes
  useEffect(() => {
    if (generatedHtml) {
  ...
  useEffect(() => {
    if (data.html) {
      try {
        const extractedData = parseTemplate(data.html);
  ...
  const handleSectionUpdate = (updatedData: Partial<TemplateData>) => {
    if (updatedData.companyInfo) {
      // For each company section, update content based on its CSS selector if available
      const updatedSections = updatedData.companyInfo.map(section => {
        if (section.cssSelector && previewDocument) {
          const element = previewDocument.querySelector(section.cssSelector);
          if (element) {
            return {
              ...section,
    ...
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-5">
          ...
          {!template && (
            <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-dashed">
