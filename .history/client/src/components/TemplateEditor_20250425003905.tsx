...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  // Handle SVG edits for company sections
  ...
  const generatePreview = (templateData: TemplateData) => {
    if (template?.html) {
      try {
        ...
        // Helper function to update HTML content while preserving structure
        const updateHtmlContent = (selector: string, content: string) => {
          const element = doc.querySelector(selector);
          if (element) {
            // Only update if content has changed
            if (element.innerHTML.trim() !== content.trim()) {
        ...
        if (templateData.title) {
          updateHtmlContent('.product-title, .product-name', templateData.title);
        }
        if (templateData.description) {
          updateHtmlContent('.product-description, .description-text', templateData.description);
        }
        if (templateData.price) {
          updateHtmlContent('.product-price', templateData.price);
        }
        ...
        if (specsContainer && templateData.specs.length > 0) {
          ...
          `).join('');
          updateHtmlContent('.specs-container, .specifications', specsHtml);
        }
        ...
        if (companySections.length > 0 && templateData.companyInfo.length > 0) {
          companySections.forEach((section, index) => {
            if (index < templateData.companyInfo.length) {
              ...
              // Update icon/svg if it exists
              const iconContainer = section.querySelector('.icon-container');
              if (iconContainer && data.svg) {
                updateHtmlContent('.icon-container', data.svg);
              }
              
              // Update title and description
              const title = section.querySelector('h3, .company-title');
              if (title) {
                updateHtmlContent('h3, .company-title', data.title);
              }
              
              const desc = section.querySelector('p, .company-description');
              if (desc) {
                updateHtmlContent('p, .company-description', data.description);
              }
            }
          });
        }
