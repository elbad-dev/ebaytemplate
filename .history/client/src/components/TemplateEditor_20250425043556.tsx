...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  const handleSectionUpdate = (updates: Partial<TemplateData>) => {
    ...
    if (updates.company_name) {
      updateTextContent('.company-name', updates.company_name);
    }
    if (updates.subtitle) {
      updateTextContent('.company-slogan', updates.subtitle);
    }
    ...
    if (updates.title || updates.price || updates.currency) {
      ...
      const titleElement = doc.querySelector('.product-title, .product-name');

      if (titleElement && updates.title) {
        titleElement.textContent = updates.title;
      }
      
      if (priceElement && (updates.price || updates.currency)) {
        const currencySymbol = getCurrencySymbol(updates.currency || templateData.currency);
        priceElement.textContent = `${currencySymbol}${updates.price || templateData.price}`;
      }
    }
    ...
    if (updates.description) {
      const descElement = doc.querySelector('.product-description, .description-text');
      if (descElement) {
        descElement.innerHTML = updates.description;
      }
    }

    // Update company logo
    if (updates.logo) {
      const logoElement = doc.querySelector('.company-logo');
      if (logoElement) {
        if (updates.logo.includes('<svg')) {
          logoElement.innerHTML = updates.logo;
        } else {
          const imgElement = doc.createElement('img');
          imgElement.src = updates.logo;
    ...
    if (updates.specs) {
      const techContainer = doc.querySelector('.tech-data .card:first-child .tech-list, .specs-container');
    ...
    if (updates.companyInfo) {
      const companyContainer = doc.querySelector('.info-cards, .company-sections');
      if (companyContainer && updates.companyInfo.length > 0) {
        companyContainer.innerHTML = updates.companyInfo.map(info => `
          <div class="card company-section" ${info.id ? `id="${info.id}"` : ''}>
            <div class="info-icon icon-container">${info.svg || ''}</div>
