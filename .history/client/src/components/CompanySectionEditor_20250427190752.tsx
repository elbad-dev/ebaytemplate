...
export default function CompanySectionEditor({ sections, onChange }: CompanySectionEditorProps) {
  ...
  // Add a new section with proper structure
  const addSection = () => {
    const newSection = {
      id: generateId(),
      title: '',
  ...
  // Remove section while preserving others
  const removeSection = (id: string) => {
    onChange(sections.filter(section => section.id !== id));
  };

  // Update section with proper structure preservation
  const updateSection = (id: string, key: keyof CompanySection, value: string) => {
    const updatedSections = sections.map(section => {
      if (section.id === id) {
        const updatedSection = { ...section, [key]: value };
        // Ensure SVG has proper structure if updating SVG
        if (key === 'svg' && !value.includes('svg')) {
          updatedSection.svg = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${value}</svg>`;
        }
                  ...
                  <div className="mt-3 text-center">
                    <IconSelector
                      ...
                      onColorChange={(color) => {
                        ...
                        // Update both stroke and fill colors
