...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  const generatePreview = async (templateData: Partial<TemplateData>) => {
    ...
    try {
      ...
      if (title && templateData.title) {
        title.textContent = templateData.title;
      }
      const companyName = doc.querySelector('.company-name');
      if (companyName && templateData.company_name) {
        companyName.textContent = templateData.company_name;
      }

      // Update price with currency
      if (templateData.price) {
        const currencySymbol = getCurrencySymbol(templateData.currency || 'USD');
        const priceEl = doc.querySelector('.price');
        if (priceEl) {
          priceEl.textContent = `${currencySymbol}${templateData.price}`;
        }
      }

      // Update product description
      const description = doc.querySelector('.product-description');
      if (description && templateData.description) {
        description.innerHTML = templateData.description;
      }

      // Update tech specs list
      const techList = doc.querySelector('.tech-list');
      if (techList && templateData.specs) {
      ...
      companySections.forEach((section, index) => {
        if (templateData.companyInfo && index < templateData.companyInfo.length) {
      ...
      if (templateData.images && templateData.images.length > 0) {
        ...
        html = replaceGallerySection(html, templateData.images);
      } else {
        html = doc.documentElement.outerHTML;
      }
