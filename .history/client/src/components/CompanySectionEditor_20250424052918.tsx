...
export default function CompanySectionEditor({ sections, onChange }: CompanySectionEditorProps) {
  ...
  // Add a new empty section with consistent structure
  const addSection = () => {
    const newSection = {
      id: generateId(),
      title: 'New Section',
    ...
    // Ensure we maintain array immutability
    onChange([...sections, newSection]);
  };
  
  // Remove a section by ID while maintaining others
  const removeSection = (id: string) => {
    onChange(sections.filter(section => section.id !== id));
  };
  
  // Update a section property while preserving structure
