...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ template, onBack }) => {
  ...
  // Parse template on mount or when template prop changes (only run on initial load or template change)
  useEffect(() => {
    if (template) {
      try {
  ...
  }, [template]); // Remove toast from deps to avoid rerun on every render
  ...
  const generatePreview = (templateData: TemplateData) => {
    if (template?.html) {
      try {
        ...
        // Update specifications (preserve style/structure)
        ...
        if (specsContainer && templateData.specs.length > 0) {
          // Try to find a template spec item to clone for style
          const templateSpec = specsContainer.querySelector('.spec-item, .tech-item, .spec-row');
          // Remove all children but keep container
          while (specsContainer.firstChild) specsContainer.removeChild(specsContainer.firstChild);
          templateData.specs.forEach(spec => {
            let newSpec;
            if (templateSpec) {
              newSpec = templateSpec.cloneNode(true);
              // Try to update label/value in a robust way
              ...
              // Fallback: if not found, just set text
              if (!labelEl && !valueEl) newSpec.textContent = `${spec.label}: ${spec.value}`;
            } else {
              // Fallback: create a simple div
              newSpec = doc.createElement('div');
              newSpec.className = 'spec-item';
              newSpec.innerHTML = `<span class="spec-label">${spec.label}</span>: <span class="spec-value">${spec.value}</span>`;
            }
            specsContainer.appendChild(newSpec);
          });
        }

        // Update company sections (preserve style/structure)
